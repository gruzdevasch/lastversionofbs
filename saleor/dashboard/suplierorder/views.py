...
def order_list(request):
    orders = SuplierOrder.objects.all()
    order_filter = SuplierOrderFilter(request.GET, queryset=orders)
    ...
    return TemplateResponse(request, 'dashboard/suplierorder/list.html', ctx)
...
def order_create(request):
    ...
    return redirect('dashboard:suplierorder-details', order_pk=order.pk)
...
def create_order_from_draft(request, order_pk):
    ...
    if form.is_valid():
        ...
        status = 400
    template = 'dashboard/suplierorder/modal/create_order.html'
...
def order_details(request, order_pk):
    qs = SuplierOrder.objects.all()
    order = get_object_or_404(qs, pk=order_pk)
    captured = ZERO_MONEY
    orderlines = order.items.all()
    balance = captured - order.total_net
    ctx = {
        'order': order,
        'captured': captured, 'balance': balance,
        'lines':orderlines}
    return TemplateResponse(request, 'dashboard/suplierorder/detail.html', ctx)
...
def order_add_note(request, order_pk):
    ...
    template = 'dashboard/suplierorder/modal/add_note.html'
    return TemplateResponse(request, template, ctx, status=status)
...
def capture_payment(request, order_pk, payment_pk):
    ...
    return TemplateResponse(request, 'dashboard/suplierorder/modal/capture.html', ctx,
                            status=status)
